    ...
    try:
        ...
        for index in range(0, len(validIPs), steps):
            ...
            try:
                ...
                for ip in ipsList:
                    writer.writerow({'IP': str(ip), 'Categories': str(
                        category), 'ReportDate': str(reportedDate), 'Comment': str(comment)})
...
def submitData(filename, APIKey):
    ...
    try:
        ...
        response = requests.post(
            'https://api.abuseipdb.com/api/v2/bulk-report', headers=headers, files=files)
        ...
        if response.status_code == 200:
            ...
            if "errors" in resposeData.keys():
                ...
                print("Saved reports: {}".format(
                    resposeData.get('data').get('savedReports')))
                ...
                if len(errors) > 0:
                    ...
                    for errorData in errors:
                        print("{} - {}".format(errorData.get('input'),
                              errorData.get('error')))
